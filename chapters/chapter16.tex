...
In view of the Riesz-Fr\'echet Representation Theorem,
\[
    \{u_n\}\rightharpoonup u\text{ in }H\quad\iff\quad\lim_{n\to\infty}\langle h,u_n\rangle=\langle h,u\rangle\quad\text{for all }h\in H.
\]
...
In this section $H$ is an inner product space.
\begin{namedthm*}{Definition}
    A subset $S$ of $H$ is said to be \textbf{orthogonal} provided every two vectors in $S$ are orthogonal.
    If such as set has the further property that each vector in $S$ is a unit vector, then $S$ is said to be \textbf{orthonormal}.
\end{namedthm*}

\begin{namedthm*}{The General Pythagorean Identity}
    If $u_1,\dots,u_n$ are $n$ orthonormal vectors in $H$, and $\alpha_1,\dots,\alpha_n$ are real numbers, then 
    \[
        \|\alpha_1u_1+\dots+\alpha_nu_n\|^2=|\alpha_1|^2+\dots+|\alpha_n|^2.
    \]
\end{namedthm*}
This identity follows easily from 
\[
    \|\alpha_1u_1+\dots+\alpha_nu_n\|^2=\langle\alpha_1u_1+\dots+\alpha_nu_n,\alpha_1u_1+\dots+\alpha_nu_n\rangle=\sum_{i=1}^n\sum_{j=1}^n\alpha_i\alpha_j\langle u_i,u_j\rangle.
\]
