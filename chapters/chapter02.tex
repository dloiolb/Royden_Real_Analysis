...
\begin{enumerate}
	...
	\end{align*}
	Then $[0,1]\subseteq A\cup([0,1]\cap\mathbb{Q})$.\\
	Taking the outer measure,
	\begin{align*}
	...
	Suppose that $\sum_{k=1}^n m^*(I_k) < 1$.
	Then by countable subadditivity of outer measure,
	\[
		m^*(\bigcup_{k=1}^n I_k)\le\sum_{k=1}^n m^*(I_k) < 1,
	\]
	and it is not possible that $[0,1]\subseteq\bigcup_{k=1}^n I_k$ by monotonicity of outer measure: $1=m^*([0,1])\le m^*(\bigcup_{k=1}^n I_k)$.\\
