...
\begin{flushleft}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item $\bigcap S$ is a $\sigma$-algebra by Lemma 2,
...
\begin{enumerate}
	...
	\begin{enumerate}[label=(\roman*),align=left]                                                                                                         
        ...
        Let $F$ be an $F_\sigma$ set, that is, $F=\bigcup_{n=1}^\infty F_n$, $F_n$ closed. \\
		For any real number $y$,
        \[
			F+y=\bigcup_{n=1}^\infty F_n+y
		\]
        \item the translate of a $G_\delta$ set is also $G_\delta$,\\
        Let $\mathcal{O}$ be a $G_\delta$ set, that is, $\mathcal{O}=\bigcap_{n=1}^\infty \mathcal{O}_n$, $\mathcal{O}_n$ open.\\
        For any real number $y$,
		\[
